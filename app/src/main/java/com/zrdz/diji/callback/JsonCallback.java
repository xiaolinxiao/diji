package com.zrdz.diji.callback;
...
//                .params("token", "3215sdf13ad1f65asd4f3ads1f");
    ...
    public T parseNetworkResponse(Response response) throws Exception {
        ...
        final String dataList = jsonObject.optString("dataList", null);
        final String message = jsonObject.optString("message", "");//每个人写的接口不同，所以判断的参数比较多
        final String data = jsonObject.optString("data", null);
        //这么多的乱，哎
        if (status.equals("TRUE")) {
            if (clazz == String.class) return (T) result;
            ...
            //下面这一行是专门为新的登陆接口写的，如果其他地方报错，这里改一下
            if (clazz != null && result != "") return new Gson().fromJson(result, clazz);
            if (clazz != null) return new Gson().fromJson(message, clazz);
            if (type != null && data == null) return new Gson().fromJson(result, type);
            if (type != null && data != null) return new Gson().fromJson(data, type);
        } else if (status.equals("false")) {
